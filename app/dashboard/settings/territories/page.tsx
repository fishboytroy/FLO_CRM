    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <h2 className="mt-2 text-2xl font-bold text-slate-900 sm:text-3xl">ZIP code territories</h2>
          <p className="mt-2 max-w-2xl text-sm text-slate-600">
            Manage purchased ZIP territories for {activeOrg.name}. These records do not route leads or control billing yet.
          </p>
        </div>
        <Link className="inline-flex min-h-11 items-center justify-center rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50" href="/dashboard/settings" prefetch={false}>
          Back to settings
        </Link>
      </div>
      ...
      {!canManage ? (
        ...
        <>
          <Card className="p-4 sm:p-6">
            <h3 className="text-lg font-bold">Add territory</h3>
            <p className="mt-2 text-sm text-slate-600">ZIP+4 values are normalized to five digits. Active and trialing exclusive territories cannot overlap.</p>
            <div className="mt-5">
              <AddTerritoryForm />
            </div>
          </Card>
              ...
              <>
                <div className="grid gap-3 p-4 md:hidden">
                  {territories.map((territory) => (
                    <div key={territory.id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">ZIP</p>
                          <p className="mt-1 text-lg font-bold text-slate-900">{territory.zipCode}</p>
                        </div>
                      ...
                      <div className="mt-4 grid gap-3 text-sm text-slate-600">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <span className="font-semibold text-slate-800">{territory.exclusive ? "Yes" : "No"}</span>
                        </div>
                        <div className="flex items-center justify-between gap-3">
                          <span className="text-xs font-semibold uppercase tracking-wide text-slate-400">Starts</span>
                          <span className="font-semibold text-slate-800">{format(territory.startsAt, "MMM d, yyyy")}</span>
                        </div>
                        <div className="flex items-center justify-between gap-3">
                          <span className="text-xs font-semibold uppercase tracking-wide text-slate-400">Expires</span>
                          <span className="font-semibold text-slate-800">{territory.expiresAt ? format(territory.expiresAt, "MMM d, yyyy") : "-"}</span>
                        </div>
                ...
                <table className="hidden w-full text-left text-sm md:table">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {territories.map((territory) => (
                      <tr key={territory.id} className="align-top">
                        <td className="px-4 py-4 font-bold text-slate-900">{territory.zipCode}</td>
